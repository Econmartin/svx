...
| Manual-claim blindness (found 2026-07-05) | in POLY_1271/Safe/Proxy modes the bot can never submit `redeemPositions` itself — the EOA can sign order messages on the smart wallet's behalf, but can't be `msg.sender` for an on-chain redeem — so the ONLY redemption path is the operator clicking "Claim" on polymarket.com (or Polymarket's auto-redeem setting); the ledger had no way to observe that, so `/status` kept reporting real, already-claimed money as unredeemed (and it silently fed the reconciliation drift alarm) | on-chain balance check (`reconcileExternallyRedeemedPositions`) at boot and on every balance refresh: for every row still marked unredeemed, read the funder's actual ERC1155 balance for that outcome token; balance=0 with no early-exit sale on record means it was claimed outside the bot — mark it redeemed (`external-claim` sentinel) so the ledger matches reality |
| Auto-redeem vs. drift race (found 2026-07-05, same day) | the redeem-blindness fix above first shipped on an hourly timer, decoupled from the 60s wallet-vs-ledger drift check — with Polymarket auto-redeem enabled, a claim landing in that up-to-59s gap looked like unexplained wallet growth and the drift alarm paused the bot on money it was itself about to reconcile away. The balance refresh (and thus the drift check) also only ran on loop ticks with ≥1 active oracle/poly match, so during quiet stretches neither check ran at all | the on-chain reconcile now runs synchronously, first, inside the SAME balance-refresh block the drift check reads from — no gap for a fresh auto-redeem to be misread as drift. That block was also moved earlier in the loop, before the match-count early return, so it fires on every tick regardless of match availability |
| Settlement-latency false positive (found 2026-07-06) | the ledger marks an early-exit sell or CTF redeem "closed" the instant the call returns success, but the actual pUSD credit can take a poll cycle to land on-chain (observed live: a $5.56 early-exit proceeds landed one 60s cycle after the ledger update, tripping a −$5.71 drift pause that self-healed to −$0.29 by the next check with no code fix or rebaseline) | the drift check now requires **2 consecutive** breaches (~60-120s apart) before pausing — logged as `drift_unconfirmed` (warn) on the 1st breach, only escalating to a pause on the 2nd. A real, persistent booking bug still trips within ~2 minutes; a single-poll settlement-latency blip clears on its own and never pauses trading |
...
| Execution slippage (found 2026-07-09 via clob prices-history) | entries and exits were market FOK orders that swept whatever the book quoted — stops realized 5–10pp below the tape on thin books (the −50% trigger routinely booked −55…−60%), and thin-book entries could walk up past the edge that justified the trade | **exit ladder**: early exits are floor-priced FAK sells — fill everything at ≥ (best bid − `polyExitMaxSlippagePts`, default 2¢), cancel the rest, retry the remainder next tick against a fresh book; a partial fill splits the ledger row (`splitPolyPartialExit`) so realized PnL books immediately and the reconciliation invariant stays exact. **Entry cap**: FOK buys carry a price cap of ask + `polyEntryMaxSlippagePts` — a thin book fails the order (skip the tick) instead of overpaying |
