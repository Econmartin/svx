...
| Range-ladder | **SIMULATED + on-demand mint** | Idea bank #1 with the strike-width question answered by data: σ/2-width rungs return +10.1% (104 oracles replayed; ATM rung +29%, wings negative — the calibration finding from a new angle). `GET /range-sim` replays it server-side; `svx mint-ladder` mints the winning ladder live via `predict::mint_range`. Added 2026-07-11. |
| PLP + tail hedge | **SIMULATED, verdict: NO** | Idea bank #2, priced with real data on both legs: PLP realizes ~+0.7%/yr (the vault is the counterparty to the calibration edge) and 1.5σ crash insurance costs ~2× its realized payout — net APY negative at any coverage. 3σ tails look underpriced (n=2, thin). `GET /plp-sim`; `svx supply-plp` for a live house-side position. Added 2026-07-11. |
